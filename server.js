...
function isLegitEdge(raw) {
  ...
  return ARROW_GLYPH.test(raw.trim());
}
...
function measureDepth(node, kidsMap, memo = {}, visiting = new Set()) {
  ...
  let depth = 1;
  for (const k of sprouts) {
    depth = Math.max(depth, 1 + measureDepth(k, kidsMap, memo, visiting));
  }
...
vessel.post("/bfhl", (req, res) => {
  try {
    ...
    for (const chunk of incoming) {
      ...
      const match = trimmed.match(ARROW_GLYPH);
      if (!match) {
        illegalBin.push(chunk);
        continue;
      }

      const [, papa, bambino] = match;
    ...
    for (const clusterNodes of Object.values(clusters)) {
      ...
      for (const node of clusterNodes) {
        if (kidsOf[node]) {
          localKids[node] = kidsOf[node].filter(k =>
            clusterNodes.has(k)
          );
        }
      }

      const rootCandidates = [...clusterNodes].filter(
        n => parentOf[n] === undefined || !clusterNodes.has(parentOf[n])
      );
      ...
      if (hasCycle) {
        ...
        const treeBlob = {
          [chosenRoot]: sculpt(chosenRoot, localKids),
        };
...
vessel.listen(DOCK, "0.0.0.0", () => {
  console.log(`BFHL vessel sailing on port ${DOCK}`);
});